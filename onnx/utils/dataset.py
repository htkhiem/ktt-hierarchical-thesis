...
def preprocess_classes(data, original_name, depth, verbose=False):
    """Create bidirectional mappings for later numerical encoding/decoding."""
    cls2idx = []
    idx2cls = []
...
class CustomDataset(torch.utils.data.Dataset):
    """The hierarchical dataset for use with all DistilBERT models."""
    ...
    def __len__(self):
        """Return number rows in the dataset."""
    ...
    def __getitem__(self, index):
        ...
        inputs = self.tokenizer(
            ...
            max_length=self.max_len,  # For us it's a hyperparam.
            padding='max_length',
            truncation=True
            # BERT tokenisers return attention masks by default
        )
        labels = self.labels.loc[index]
        result = {
            'ids': torch.tensor(
            ...
            'labels_b': torch.tensor(
                self.labels_b.iloc[index], dtype=torch.long),
        }
        if self.weights is not None:
            result['weights'] = np.float32(self.weights[labels[-1]])
        return result
...
def get_loaders(
    ...
    # binary: Whether to genenrate bittsnary vector encodings or not (for C-HMCNN
    #   and DB-FBHCN+AWX)
    ...
    # balanced: Export class population balancing weights to help with severely
    #   imbalanced data.
    ...
    if not full_set:
        filtered = data.sample(
            frac=partial_set_frac, random_state=RANDOM_SEED
        )[columns]
    else:
    ...
    # Generate population balancing weights based on leaf level.
    weights = None
    if balanced:
        weights = compute_class_weight(
            'balanced',
            classes=np.arange(
                start=0, stop=len(idx2cls[-1]), step=1, dtype=np.int
            ),
            y=data['codes'].apply(lambda lst: lst[min(depth-1, len(lst)-1)])
        )
        if verbose:
            print('Rebalancing classes with weights:', weights)

    train_set = CustomDataset(
        train_set, hierarchy, distilbert.tokenizer, 64,
        text_col_name=input_col_name, weights=weights
    )
    val_set = CustomDataset(
        val_set, hierarchy, distilbert.tokenizer, 64,
        text_col_name=input_col_name, weights=weights
    )
    test_set = CustomDataset(
        test_set, hierarchy, distilbert.tokenizer, 64,
        text_col_name=input_col_name, weights=weights
    )

    train_loader = torch.utils.data.DataLoader(
        dataset=train_set, batch_size=config['train_minibatch_size'],
        shuffle=True, num_workers=0
    )
    val_loader = torch.utils.data.DataLoader(
        dataset=val_set, batch_size=config['val_test_minibatch_size'],
        shuffle=True, num_workers=0
    )
    test_loader = torch.utils.data.DataLoader(
        dataset=test_set, batch_size=config['val_test_minibatch_size'],
        shuffle=True, num_workers=0
    )
